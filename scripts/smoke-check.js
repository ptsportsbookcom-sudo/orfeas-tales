...
function checkApp() {
  ...
  if (!/DYNAMIC_WAL_STORY_IDS\s*=\s*Object\.keys\(storyImages\s*\|\|\s*\{\}\)/.test(app) || !/filter\(storyId\s*=>\s*storyId\s*>=\s*5\)/.test(app) || !/usesDynamicWalStory\(walStoryId\)/.test(app) || !/renderDynamicComicPages\(walStoryId/.test(app) || !/comic-dynamic-nav/.test(app)) {
    fail("app.js must automatically configure data-driven Watch & Listen navigation for later stories.");
  }
}
